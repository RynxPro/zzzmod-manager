...
import { Upload, FolderOpen, ArrowUpDown, Power, Trash2, Search, Plus } from "lucide-react";
...
export const ModsPage: FC<ModsPageProps> = ({ initialMods = [], initialCharacter = null }) => {
  ...
  useEffect(() => {
    // Ensure initialMods is an array before setting it
    const safeInitialMods = Array.isArray(initialMods) ? initialMods : [];
    setMods(safeInitialMods);
    
    if (initialCharacter) {
      setSelectedCharacter(initialCharacter);
    }
    
    // Only set loading to false if we actually have initialMods
    if (initialMods) {
      setLoading(false);
    }
  ...
  const filteredAndSortedMods = useMemo(() => {
    ...
    return [...filtered].sort((a, b) => {
      ...
      let cmp = 0;
      if (sortBy === "name") {
        cmp = String(getValue(a, 'name')).localeCompare(String(getValue(b, 'name')));
      } else if (sortBy === "date") {
  ...
  const toggleMod = useCallback(async (modId: string, enabled: boolean) => {
    try {
  ...
  const handleImport = useCallback(async () => {
    ...
    try {
      ...
      success(`Mod imported successfully${character ? ` for ${character}` : ''}!`);
    } catch (error) {
      console.error('Import error:', error);
      showError(error instanceof Error ? error.message : 'Failed to import mod');
    } finally {
  ...
  const handleImportClick = useCallback(async (type: 'zip' | 'folder' | 'file') => {
    try {
      ...
      showError(error instanceof Error ? error.message : 'Failed to open import dialog');
    }
  ...
  const handleCharacterSelect = useCallback((character: string | null) => {
  ...
  const handleImportButtonClick = useCallback((type: 'zip' | 'folder' | 'file') => {
    ...
    <div className="min-h-screen bg-gaming-bg text-gaming-text">
      ...
      <div className="p-8 space-y-6">
            ...
            <div className="flex items-center space-x-3">
              ...
              {selectedCharacter && (
                <div className="text-sm text-gaming-text-secondary">
                  Adding to: <span className="font-medium text-gaming-accent-violet">{selectedCharacter}</span>
                </div>
              )}
            </div>
        ...
        <div className="relative min-h-[400px]">
          ...
          ) : !Array.isArray(filteredAndSortedMods) || filteredAndSortedMods.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-center p-8">
              <div className="bg-gaming-panel/50 rounded-full p-6 mb-4">
                <FolderOpen size={48} className="text-gaming-text-secondary mx-auto" />
              </div>
              <h3 className="text-xl font-medium text-gaming-text mb-2">No mods found</h3>
                      ...
                      <div className="relative pt-[56.25%] bg-gaming-card-dark overflow-hidden">
                        {mod.thumbnailPath ? (
                          ...
                          <div className="absolute inset-0 flex items-center justify-center bg-gradient-to-br from-gaming-card-dark to-gaming-card-darker">
                            <FolderOpen size={48} className="text-gaming-text-secondary opacity-50" />
                          </div>
                        )}
                      </div>
